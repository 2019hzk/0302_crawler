...
class BilibiliLogin(AbstractLogin):
    ...
    async def begin(self):
        ...
        if config.LOGIN_TYPE == "qrcode" or config.LOGIN_TYPE == "phone":
            utils.logger.info("[BilibiliLogin.begin] Opening bilibili.com for manual login, please log in within 2 minutes ...")
            await self.context_page.goto("https://www.bilibili.com")
            try:
                await self.check_login_state()
            except RetryError:
                utils.logger.info("[BilibiliLogin.begin] login failed please confirm ...")
                sys.exit()
