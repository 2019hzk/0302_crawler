...
class WeiboCrawler(AbstractCrawler):
    ...
    def __init__(self):
        ...
        self.cookie_urls = [self.mobile_index_url, self.index_url, "https://passport.weibo.com"]
    ...
    async def start(self):
        ...
        async with async_playwright() as playwright:
            manual_login_headless = config.HEADLESS
            manual_login_cdp_headless = config.CDP_HEADLESS
            if config.LOGIN_TYPE in ("qrcode", "phone") and config.SAVE_LOGIN_STATE:
                manual_login_headless = False
                manual_login_cdp_headless = False
                if config.HEADLESS or config.CDP_HEADLESS:
                    utils.logger.info("[WeiboCrawler] Manual login mode requires a visible browser; disabling headless for this run")
            ...
            if config.ENABLE_CDP_MODE:
                ...
                self.browser_context = await self.launch_browser_with_cdp(
                    ...
                    headless=manual_login_cdp_headless,
                )
            else:
                utils.logger.info("[WeiboCrawler] Launching browser with standard mode")
                # Launch a browser context.
                chromium = playwright.chromium
                self.browser_context = await self.launch_browser(chromium, None, self.mobile_user_agent, headless=manual_login_headless)
            ...
            if not await self.wb_client.pong():
                ...
                # After successful login, redirect to mobile website and update cookies
                utils.logger.info("[WeiboCrawler.start] redirect weibo mobile homepage and update cookies")
