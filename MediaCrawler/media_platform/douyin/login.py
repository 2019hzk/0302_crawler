...
class DouYinLogin(AbstractLogin):
    ...
    async def begin(self):
        ...
        if config.LOGIN_TYPE == "qrcode" or config.LOGIN_TYPE == "phone":
            utils.logger.info(f"[DouYinLogin.begin] Opening douyin.com for manual login, please log in within 2 minutes ...")
            ...
            # cookie 登录快速验证，不进入 10 分钟重试循环
            for _ in range(10):
                if await self.check_login_state():
                    break
                await asyncio.sleep(1)
            else:
                utils.logger.info("[DouYinLogin.begin] cookie login failed, cookies may be expired ...")
                sys.exit()
            utils.logger.info(f"[DouYinLogin.begin] cookie login successful ...")
            return
        else:
