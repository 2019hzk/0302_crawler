...
class BaiduTieBaLogin(AbstractLogin):
    ...
    async def begin(self):
        ...
        if config.LOGIN_TYPE == "qrcode" or config.LOGIN_TYPE == "phone":
            utils.logger.info("[BaiduTieBaLogin.begin] Opening tieba.baidu.com for manual login, please log in within 2 minutes ...")
            await self.context_page.goto("https://tieba.baidu.com")
            try:
                await self.check_login_state()
            except RetryError:
                utils.logger.info("[BaiduTieBaLogin.begin] login failed please confirm ...")
                sys.exit()
